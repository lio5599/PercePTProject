...
class FrontLunge(Exercise):
    def __init__(self, target_angle=90.0):
        super().__init__("Front Lunge")
    ...
    #process landmarks for front lunge specifically
    def process_landmarks(self, landmarks, mp_pose):
        ...
        if hip.visibility > 0.5 and knee.visibility > 0.5 and ankle.visibility > 0.5:
            ...
            raw_angle = self.calculate_angle(hip_coord, knee_coord, ankle_coord)
    ...
    def update_fsm(self, knee_angle):
        if self.current_state == 'IDLE':
            ...
            if knee_angle > self.min_angle_reached + 10.0:
                ...
                elif (-10.0 <= angle_diff < -5.0) or (10.0 < angle_diff <= 20.0):
